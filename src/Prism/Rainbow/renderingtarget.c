...
static void s_Pr_ApplyTexture(Pr_RenderingTarget * ap_target, Pr_TextureRef ap_tex)
{
    ...
    Pr_BindTexture(ap_tex,PR_FALSE);
...
static void s_Pr_ApplyShader(/* Pr_ShaderRef */ void * ap_shader)
{
    /*
        Pr_BindShader(ap_shader)
    */
}

pr_bool_t Pr_GetRndTargetViewport(Pr_RenderingTargetRef ap_target, Pr_ViewRef ap_view, Pr_Recti * ap_dst)
{
    float l_w;
    float l_h;
    Pr_Rectf lp_vp;
    ...
    ap_dst->x       = (int)(.5f + l_w * lp_vp.x);
    ap_dst->y       = (int)(.5f + l_h * lp_vp.y);
    ap_dst->width   = (int)(.5f + l_w * lp_vp.width);
    ap_dst->height  = (int)(.5f + l_h * lp_vp.height);
...
void Pr_ResetGLStates(Pr_RenderingTarget * ap_target)
{
    ...
    s_Pr_ApplyTexture(ap_target,NULL);
    s_Pr_ApplyShader(NULL);

    ap_target->cache.useVertexCache = PR_FALSE;

    Pr_SetRndTargetView(ap_target,&ap_target->defaultView);
}
    ...
    )
{
    ...
    if (!ap_states) {
        ...
        memcpy(&l_states, ap_states, sizeof(Pr_RenderingStates));
    }
    ...
    l_useVertexCache = (a_type <= PR_VERTEXCACHE_SIZE);
    if (l_useVertexCache) {
        pr_u32_t l_i;
    ...
    if (l_states.shader) {
        s_Pr_ApplyShader(l_states.shader);
    }
    ...
    if (ap_vertices) {
        Pr_ByteArrayRef lp_bytes = (Pr_ByteArrayRef)ap_vertices;
